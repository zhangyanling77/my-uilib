...
if (!fs.existsSync(path.resolve(__dirname, '../../my-uilib'))) {
  console.error('未找到迁移目标项目：my-uilib，请检查后重试');
  return;
}
...
compPaths.forEach(pathItem => {
  const targetPath = pathItem.replace('/my-component/src/components', '/my-uilib/src');
  try {
